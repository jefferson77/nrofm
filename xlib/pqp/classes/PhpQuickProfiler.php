...
class PhpQuickProfiler {
	...
	public function __construct($startTime, $config = '/nro/xlib/pqp/') {
	...
	public function gatherQueryData() {
		...
		if(count($sqlQueries) > 0) {
			$queryTotals['count'] = count($sqlQueries);
			foreach($sqlQueries as $key => $query) {
				if (substr($query['sql'], 0, 6) == 'SELECT') $query = $this->attemptToExplainQuery($query);
	...
	function attemptToExplainQuery($query) {
		...
		if (substr($query['sql'], 0, 6) == 'SELECT') {
			$query['explain'] = $DB->getRow('EXPLAIN '.$query['sql']);
		}
